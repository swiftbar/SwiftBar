...
extension NSColor {
    ...
    public static func webColor(from colorString: String?) -> NSColor? {
        ...
        if colorString.hasPrefix("#") {
            return colorWithHexColorString(colorString: colorString)
        }

        if let color = NSColor.cssColors[colorString] {
            return colorWithHexColorString(colorString: color)
        }

        return nil
    }

    static func colorWithHexColorString(colorString: String) -> NSColor? {
        let colorString = colorString.hasPrefix("#") ? String(colorString.dropFirst()):colorString
